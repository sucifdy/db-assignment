...
func (u *sessionsRepoImpl) UpdateSessions(session model.Session) error {
	_, err := u.db.Exec(
		"UPDATE sessions SET token = $1, expiry = $2 WHERE username = $3",
...
func (u *sessionsRepoImpl) SessionAvailName(name string) error {
	row := u.db.QueryRow("SELECT id FROM sessions WHERE username = $1", name)

	var id int
	err := row.Scan(&id)
	if err == sql.ErrNoRows {
		return fmt.Errorf("session not found for user: %s", name)
	}
	return err
}
